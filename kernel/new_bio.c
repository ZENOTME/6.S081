...
struct {
  ...
  ///////////////////////////////
  struct entry entry[13];
  struct spinlock entry_lock[13];
...
binit(void)
{
  ...
  for(int i=0;i<13;i++){
	char name[13];
	snprintf(name,13,"bcache_lock%d",i);
  	initlock(&bcache.entry_lock[i], name);
  }
...
brelse(struct buf *b)
{
  ...
  acquire(&bcache.lock);
  acquire(&bcache.entry_lock[b->blockno%13]);
  b->refcnt--;
  if (b->refcnt == 0) {
    blockno=b->blockno;
  ...
  release(&bcache.entry_lock[b->blockno%13]);
  release(&bcache.lock);
